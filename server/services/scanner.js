  ...
  try {
    ...
    if (!fs.existsSync(documentsPath)) {
      return Promise.reject(
        new Error('Documents path does not exist: ' + documentsPath)
      );
    }
    ...
    var existing = {};
    db.prepare(
      'SELECT id, file_path, file_hash, file_size, file_mtime FROM documents'
    )
      .all()
    ...
    var transaction = db.transaction(function () {
      files.forEach(function (file) {
        ...
        if (!existingDoc) {
          ...
          existingDoc.file_mtime === file.mtime
        ) {
          ...
          if (existingDoc.file_hash && existingDoc.file_hash === hash) {
            ...
            db.prepare('UPDATE documents SET file_mtime = ? WHERE id = ?').run(
              file.mtime,
              existingDoc.id
            );
            unchanged++;
      ...
      Object.keys(existing).forEach(function (filePath) {
        if (!seenPaths[filePath]) {
          db.prepare('DELETE FROM documents WHERE id = ?').run(
            existing[filePath].id
          );
...
function walkDirectory(dir, rootDir) {
  ...
  try {
    ...
    log.error(
      { dir: dir, err: { message: err.message } },
    ...
    if (entry.isDirectory()) {
      ...
      if (SUPPORTED_EXTENSIONS.indexOf(ext) !== -1) {
        ...
        try {
          ...
          log.error(
            { file: fullPath, err: { message: err.message } },
            'Cannot stat file'
          );
        }
      }
...
function computePartialHash(filePath) {
  try {
    ...
    log.error(
      { file: filePath, err: { message: err.message } },
...
function getPageCount(filePath, fileType) {
  if (fileType === 'pdf') {
    return execAsync('pdfinfo "' + shellEscape(filePath) + '"').then(
      function (stdout) {
    ...
    return execAsync('unrar lb "' + escapedCbr + '"')
      ...
      .catch(function (err) {
        log.error(
          { file: filePath, fileType: 'cbr', err: { message: err.message } },
          'Archive read error'
        );
        return 0;
      });
    ...
    return execAsync('unzip -l "' + shellEscape(filePath) + '"')
      ...
      .catch(function (err) {
        var msg = err.message || '';
        if (
          msg.indexOf('End-of-central-directory') !== -1 ||
          msg.indexOf('not a zipfile') !== -1
        ) {
          log.warn(
            { file: filePath, fileType: 'cbz' },
            'Skipping invalid CBZ (not a valid ZIP)'
          );
        } else {
          log.error(
            { file: filePath, fileType: 'cbz', err: { message: msg } },
            'Archive read error'
          );
        }
        return 0;
...
function invalidateCache(docId) {
  ...
  try {
    ...
    log.warn(
      { docId: docId, err: { message: err.message } },
